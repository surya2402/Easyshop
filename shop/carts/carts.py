...
from shop.products.products import brands, categories
import json
...
def getCart():
    ...
    for key,product in session['Shoppingcart'].items():
        ...
        subtotal-=discount
        tax = ("%.2f" % (0.02*float(subtotal)))
        grandtotal=float("%.2f" %(1.02*subtotal))
    return render_template('products/carts.html',subtotal=subtotal,tax=tax,grandtotal=grandtotal,categories=categories(),brands=brands())

@app.route('/cart')
def noproduct():

    return render_template('products/noproduct.html',categories=categories(),brands=brands())
